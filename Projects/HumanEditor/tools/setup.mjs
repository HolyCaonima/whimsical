...
for(const name of ['editor','viewport','panels','main'])asset('scripts/'+name,'Script','',{storage:'external',source:name+'.js'});
...
const presets=['Afterlight','MapleCircuit'].map(name=>{const project=JSON.parse(fs.readFileSync(path.join(repo,'Projects',name,'.project'),'utf8'));return {name,content:'Projects/'+name+'/Content',scripts:project.scripts,map:project.startupMap};});
asset('Settings','Data',{content:presets[0].content,map:presets[0].map,scripts:presets[0].scripts,presets});
