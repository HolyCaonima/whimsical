...
每次求解推进 1/30 秒，4 子步 × 12 迭代。绳索 64 条为 4,096 变量 / 16,320 关系，布料 32² 为 1,024 变量 / 8,898 关系。渲染实例上限 1,024：绳索 64 条用 960 个实例（每条 12 段可视绳段，物理仍是 64 个节点），布料 32² 用 676 块薄板 + 展台，同样低于上限。面板的长度误差统计已完成样本中全部结构关系。
