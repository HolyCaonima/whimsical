...
10,000 个位置自由度组成显式粒子集合，五个平面的法线与偏移自由度组成显式边界集合。用户用 op 编写平方间距和半空间残差，再声明三条绑定：

```js
X.pair(separation, particles, particles, {diameter:0.07}, {
    self:'undirected', includeSelf:false
});
X.pair(containment, particles, walls, {radius:0.035});
X.pair(damping, particles, frame, [], {history:initial, compliance:[0.08,0.08,0.08]});
```

逻辑上分别有 49,995,000、50,000、10,000 个关系实例，共 50,055,000 个。连接和公式仍全部由这里的 DSL 定义；Compiler 根据数学界及成本自动选择完整活动扫描或网格候选，不要求项目枚举邻居。它展示间距及容器条件，不包含不可压缩水的密度模型。面板的最大重叠率只是项目诊断，使用直径网格精确检查邻格，不参与求解。`I` 给集合施加一个向上的冲量。
...
误差仪表只消费新完成的样本，不在编译等待或暂停后反复计算相同数据。小型诊断直接实现 `measure(sample)`；较重的诊断可实现 `beginMeasure(sample)`，返回带 `step()` 的任务，每次固定更新执行一段，返回数值时才发布结果。重建模型时丢弃未完成任务。

粒子诊断持有一次完整快照，按占用网格单元遍历半邻域，用最小平方距离求最大重叠率，最后只开方一次。扫描以约 2 ms 为目标在每 16 个单元之间让出执行，仪表刷新率随工作量变化；模拟步长、求解精度与渲染插值不变。2026-09-12 在 RTX 3080、Release、1440×900、immediate present 下，使用当时的 4,000 粒子配置各运行 2,400 个渲染帧并用临时 `Date.now()` 计时：原粒子诊断单次峰值 33 ms，改后诊断每次更新峰值 6 ms（包含建立网格）；这是 JS 诊断区间的墙钟采样，不是整帧或 GPU 耗时。首次建模的同步开销仍存在。
