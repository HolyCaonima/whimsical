...
全部 4,000 个粒子由一个显示实体承载：一个 Render 组件共享球体网格与材质，一个 `target:'renderInstances'` 的 Dynamics 输出绑定把位置集合映射到实例变换数组，并保留 0.07 秒姿态插值。粒子群始终只占一个排序项和每个 pass 的一个实例绘制批次，容器和灯仍是独立实体。映射由项目声明，使用框架的通用批量输出路径；模拟样本目前仍经 CPU 回读。
