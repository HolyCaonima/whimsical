...
GPU 数学约束求解由独立的 [Dynamics system](dynamics.md) 提供：Model 描述变量和关系，Compiler 生成批量计划，Runtime 持有状态并使用 RenderCore 执行。脚本前端与 GPU 服务之间采用可靠请求通道，独立于渲染 FrameMailbox 的快照替换。
...
| `Engine.parent(id,parent,keepWorld)` / `scale(id,scale)` | 设置无切变 TRS 层级／修改局部缩放 |
