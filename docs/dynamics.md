...
实例索引为 32 位；集合内的 `(set, index)` 在追加和计划切换后保持稳定，不复用旧索引。模型实例只有固定的 28 个求解 buffer 角色及少量辅助资源；十万个变量不会变成十万个 ResourceId、表达式或 pass。
...
关联结构按端点数增长，不创建关系之间的两两冲突图。Jacobi 用端点最大关联度同时缩放乘子增量与对应修正。对于可识别有限差分界的大集合，Auto 的全局静态路径按当轮活动关联数缩放；启用的等式始终计入，标量不等式在违反或乘子非零时计入，端点别名只计一次。其他计划保留静态关联缩放。活动缩放改变有限迭代的数值轨迹，空间索引自身只筛选候选。它们都不改变用户公式、柔顺度或迭代预算。
...
可运行示例位于 [ConstraintLab](../Projects/ConstraintLab/README.md)：布料、绳索、绳网、穿绳布幕、星仪，以及集合粒子示例。粒子示例仍只声明三条高层绑定，逻辑上有 124,750 个无向组合、2,500 个边界组合和 500 个阻力实例；27 个整数描述全部 257,500 个端点引用。Compiler 从平方和不等式中证明有限差分界，结合域规模选择完整活动扫描或 GPU 哈希网格，随后按数学类型压紧活动工作列表并调用原求解函数。当前 500 自由度示例选择完整活动扫描；扩大到 2,000 自由度时选择网格。两者都使用活动关联缩放，不需要用户补充邻居表或改动 DSL。参数范围更新使边界缓存失效；索引每轮在当前 Jacobi 快照上刷新，非零乘子关系继续参与。公开参数、history、乘子和编译元数据仍保留逻辑行存储，CPU 展开及其内存成本尚未消除。实现范围和实测见 [候选域说明](../engine/physics/dynamics/compiler/CandidateDomains.md)。项目面板的网格误差诊断独立于求解器。
