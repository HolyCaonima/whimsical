...
def object_at(name,p,scale=(1,1,1),material=0,visible=True):
    return {'id':uid(name),'name':name,'enabled':True,'components':{'transform':{'position':list(p),'rotation':[0,0,0,1],'scale':list(scale)},
            'render':{'material':material,'visible':visible},
            }}
...
scene={'version':11,'entities':objects,
