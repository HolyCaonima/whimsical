...
ScriptBindings::ScriptBindings(duk_context* c, std::shared_ptr<Mailbox> mailbox)
    ...
    duk_put_prop_string(c, -2, "dynamics");
    duk_pop(c);
...
__declspec(noinline) duk_ret_t ScriptBindings::dispatch(duk_context* c, int op) {
    ...
    if (op == 1) {
        ...
        if (def.contains("domain") || def.contains("projection"))
            throw std::invalid_argument("Relations declare residual conditions through kind; solver projections are not supported");
        if (def.contains("kind")) {
            auto kind = def.at("kind").string();
            if (kind == "greaterEqual")
                t->kind = RelationKind::GreaterEqual;
            else if (kind == "lessEqual")
                t->kind = RelationKind::LessEqual;
    ...
    if (e.channel->busy())
        throw std::runtime_error("Dynamics instance is busy; poll its reply before submitting another operation");
    Request request{};
    if (op == 6) {
        ...
        throw std::logic_error("Unknown Dynamics operation");
