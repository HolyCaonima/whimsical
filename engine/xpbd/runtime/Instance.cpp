...
Range range(const CompiledPlan& plan, StateField field, SetId id) {
    if (field == StateField::History || field == StateField::Multiplier) {
        const auto& layout = plan.relations.at(id);
        const auto& t = *plan.types[layout.type];
        return {field == StateField::History ? BufferRole::History : BufferRole::Multipliers,
                field == StateField::History ? layout.history : layout.multipliers, layout.count,
                field == StateField::History ? t.history : t.rows, layout.count};
    }
...
struct Instance::Storage {
    ...
    void dynamicEndpoints(const TickInput::Endpoints& input) {
        ...
        for (uint32_t e = 0; e < type.spaces.size(); ++e) {
            ...
            auto retract = expected.retract.glsl("r"), difference = expected.difference.glsl("d");
            for (uint32_t v = 0; v < plan->variables.size(); ++v) {
                ...
                compatible[v] =
                    ...
                    actual.retract.glsl("r") == retract && actual.difference.glsl("d") == difference;
            }
            for (uint32_t i = 0; i < count; ++i) {
...
Instance::Instance(rc::RenderCore& core, PlanRef plan) : core_(core) {
    if (!plan)
        throw std::invalid_argument("XPBD instance requires a compiled plan");
    model_ = plan->model;
...
void Instance::idle() const {
    if (pending_)
        throw std::logic_error("Complete the submitted XPBD tick before changing this instance");
}
void Instance::step(const TickInput& input) {
    ...
    if (input.modelVersion != model_.version || input.tick <= completed_.tick || !std::isfinite(input.dt) ||
        ...
        throw std::invalid_argument(
            "XPBD tick requires the current model version, advancing tick and positive dt");
    auto& s = *storage_;
    s.execution->graph().reset();
    for (const auto& write : input.writes) {
        if (write.field == StateField::Multiplier)
            throw std::invalid_argument("Solver multipliers are initialized by the solve plan");
...
std::vector<float> Instance::read(StateField field, SetId set, uint32_t first, uint32_t count) {
    ...
    if (first > source.count || count > source.count - first)
        throw std::out_of_range("XPBD readback range");
...
void Instance::install(PlanRef next) {
    ...
    if (!mapping.empty()) {
        ...
        const auto& mapProgram =
            target->execution->compute("Migrate XPBD state.comp", target->interface + R"(
...
if(kind==0u)x_q[dst]=x_sourceValues[src];else if(kind==1u)x_velocity[dst]=x_sourceVelocity[src];else if(kind==2u)x_history[dst]=x_sourceHistory[src];else x_acceleration[dst]=x_sourceAcceleration[src];}
        ...
        try {
            target->dispatch(mapProgram, "Migrate retained XPBD state",
