...
enum class Domain { Equality, NonNegative, NonPositive, Projected };
struct RelationType {
    std::string name;
    std::vector<SpaceRef> spaces;
    uint32_t parameters = 0, history = 0, rows = 1;
    Domain domain = Domain::Equality;
    // Inputs: endpoint states, parameters, history, candidate multipliers, dt, time.
    Formula residual;
    std::optional<Formula> projection, update;
    uint32_t stateSize() const;
    uint32_t tangentSize() const;
    uint32_t inputSize() const {
        return stateSize() + parameters + history + rows + 2;
    }
    void validate() const;
};
...
class RelationBuilder {
  ...
  public:
    ...
    Scalar multiplier(uint32_t) const;
    Scalar dt() const;
    Scalar time() const;
    Scalar constant(float v) const {
        return expression_.constant(v);
    }
    RelationRef finish(Vector residual, Domain = Domain::Equality, Vector projection = {},
                       Vector update = {}) const;
};
