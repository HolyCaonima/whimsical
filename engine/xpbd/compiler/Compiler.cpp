...
uint32_t checked(size_t n) {
    if (n > UINT32_MAX)
        throw std::overflow_error("XPBD plan exceeds 32-bit element addressing");
    return uint32_t(n);
}
template <class T> uint32_t append(BufferData& target, const std::vector<T>& values) {
    ...
    if (uint64_t(first) + values.size() > UINT32_MAX)
        throw std::overflow_error("XPBD buffer element capacity");
...
std::string signature(const Space& s) {
    return std::to_string(s.stateSize) + ":" + std::to_string(s.tangentSize) + s.retract.glsl("r") +
           s.difference.glsl("d");
}
std::string signature(const RelationType& t) {
    std::string s =
        std::to_string(t.parameters) + ":" + std::to_string(t.history) + ":" + std::to_string(int(t.domain));
    for (const auto& space : t.spaces)
        s += signature(*space);
    s += t.residual.glsl("r");
    if (t.projection)
        s += t.projection->glsl("p");
    if (t.update)
        s += t.update->glsl("u");
    return s;
}
...
PlanRef Compiler::compile(const ModelSnapshot& model, const SolverPolicy& policy) const {
    ...
    if (!policy.substeps || !policy.iterations || policy.colorBudget > 64 ||
        ...
        throw std::invalid_argument("Invalid XPBD solver policy");
